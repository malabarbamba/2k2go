...
const DUE_SUNSET_GUARD_BLOCKED_ERROR_MESSAGE =
	"Le fallback legacy du scheduler due est retire. Activez le sunset guard explicite ou le rollback global.";
const REVIEW_SUNSET_GUARD_BLOCKED_ERROR_MESSAGE =
	"Le fallback legacy du scheduler review est retire. Activez le sunset guard explicite ou le rollback global.";
...
export async function fetchDueCardsByReviewTypes(
	...
	try {
		...
		if (canUseRuntimeDueScheduler) {
			...
			let runtimeResponse: SchedulerDueResponse | null = null;
