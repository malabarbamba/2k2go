...
declare global {
	interface Window {
		__appPretextProfiler?: RuntimeProfilerApi;
	}
}

const DEFAULT_PAGE_PATH = "/app";
...
export function ensureAppRuntimeProfiler(pagePath?: string): void {
	...
	if (!window.__appPretextProfiler) {
		window.__appPretextProfiler = {
			markBaseline,
			report,
			compareWithBaseline,
			reset,
