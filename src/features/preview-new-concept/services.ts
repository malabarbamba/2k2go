...
import { normalizeLegacyAppNavigationTarget } from "@/lib/legacyAppUrl";
...
const PREVIEW_CONNECTIONS_PATH = "/app/camarades";
const LEGACY_PROFILE_ROOT_PATH = "/app-legacy/profil";
const LEGACY_PROFILE_PATH_PATTERN = /^\/app-legacy\/profil\/([^/?#]+)/i;
...
function extractLegacyProfileUsernameFromPath(
	...
	try {
		const parsed = new URL(navigationTarget, "https://arabeurgence.local");
		pathname = parsed.pathname;
	} catch {
		return null;
	}

	const match = pathname.match(LEGACY_PROFILE_PATH_PATTERN);
	if (!match || !match[1]) {
		return null;
	}
...
export function resolvePreviewNotificationDestination(
	...
	const normalizedActionTarget = item.actionUrl
		? normalizeLegacyAppNavigationTarget(item.actionUrl)
		: null;
	const usernameFromLegacyProfilePath = extractLegacyProfileUsernameFromPath(
		normalizedActionTarget,
	);
	if (usernameFromLegacyProfilePath) {
		return buildPreviewProfilePath(usernameFromLegacyProfilePath);
	}

	if (
		normalizedActionTarget === "/profil/amis" ||
		normalizedActionTarget === LEGACY_PROFILE_ROOT_PATH ||
...
function normalizePreviewNotificationPayload(
	...
	if (typeof candidate.actionUrl === "string") {
		payload.actionUrl =
			normalizeLegacyAppNavigationTarget(candidate.actionUrl) ?? undefined;
	}
