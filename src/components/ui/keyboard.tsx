...
const SIMPLIFIED_AZERTY_LAYOUT = CLAVIER_ARABE_LAYOUTS.azerty;
...
const SIMPLIFIED_AZERTY_TRANSLITERATION_KEYS = SIMPLIFIED_AZERTY_LAYOUT.rows
	.flatMap((row) => row.keys)
	.filter((key) => key.latinKey !== "space");

const SIMPLIFIED_SINGLE_CHAR_TRANSLITERATIONS: Record<string, string> =
	Object.fromEntries(
		SIMPLIFIED_AZERTY_TRANSLITERATION_KEYS.filter(
...
const NormalKeypad = () => {
	const diacriticRow = SIMPLIFIED_AZERTY_LAYOUT.diacriticsRow;
...
const SimplifiedKeypad = () => {
	const simplifiedRows = [
		SIMPLIFIED_AZERTY_LAYOUT.diacriticsRow,
		...SIMPLIFIED_AZERTY_LAYOUT.rows,
	].map((row) => ({
		...row,
