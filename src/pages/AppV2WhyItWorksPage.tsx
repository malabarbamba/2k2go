...
const APP_V2_TOC_MIN_WIDTH_PX = 220;
const APP_V2_TOC_MAX_WIDTH_PX = 300;
const APP_V2_TOC_STEP_PX = 4;
const APP_V2_TOC_ITEM_LINE_HEIGHT_PX = 18;
const APP_V2_TOC_FONT = "400 13.3333px Arial, sans-serif";
...
function toAppV2DocsPath(slug: string): string {
	const normalizedSlug = normalizeDocsSlug(slug);
	if (normalizedSlug === "/") {
		return APP_V2_DOCS_BASE_PATH;
	}

	return `${APP_V2_DOCS_BASE_PATH}${normalizedSlug}`;
}

function resolveDocsSlugFromPath(pathname: string): string {
	const normalizedPath = pathname.replace(/\/+$/g, "") || APP_V2_DOCS_BASE_PATH;
	if (
		normalizedPath === APP_V2_DOCS_BASE_PATH ||
		normalizedPath === `${APP_V2_DOCS_BASE_PATH}/`
	) {
		return "/";
	}

	if (!normalizedPath.startsWith(`${APP_V2_DOCS_BASE_PATH}/`)) {
		return "/";
	}

	const nestedPath = normalizedPath.slice(APP_V2_DOCS_BASE_PATH.length);
...
function renderInline(text: string): ReactNode[] {
	...
	for (const match of text.matchAll(pattern)) {
		...
		if (match[1] && match[2]) {
			...
			const resolvedHref = href.startsWith("/") ? toAppV2DocsPath(href) : href;
			if (isExternal) {
				parts.push(
					<a
						key={linkKey}
...
function DocsToc({ activeSlug }: { activeSlug: string }) {
	return (
		<nav aria-label="Sommaire documentation App V2">
			{WEBAPP_DOCS_NAV_CATEGORIES.map((category) => (
						...
						{category.articles.map((article) => {
							...
							return (
								<li key={article.slug} style={{ marginBottom: "2px" }}>
									<Link
										to={toAppV2DocsPath(article.slug)}
										style={{
											...LINK_STYLE,
...
function resolveCategoryForSlug(slug: string): WebappDocsCategory | null {
	for (const category of WEBAPP_DOCS_NAV_CATEGORIES) {
		if (
...
export default function AppV2WhyItWorksPage() {
	...
	const article = useMemo(
		() => getWebappDocsArticleBySlug(activeSlug),
		[activeSlug],
	);
	const category = useMemo(
		() => resolveCategoryForSlug(activeSlug),
		[activeSlug],
	);
	...
	const tocLabels = useMemo(
		() =>
			WEBAPP_DOCS_NAV_CATEGORIES.flatMap((navCategory) => [
				navCategory.label,
	...
	const preparedTocEntries = useMemo(
		() =>
			tocLabels.map((label) => ({
				label,
				prepared: prepareTextForLayout(label, APP_V2_TOC_FONT, undefined, {
	...
	const tocColumnWidthPx = useMemo(() => {
		if (layoutWidth <= 0 || layoutWidth <= 900) {
			return APP_V2_TOC_MIN_WIDTH_PX;
		}

		for (
			let width = APP_V2_TOC_MIN_WIDTH_PX;
			width <= APP_V2_TOC_MAX_WIDTH_PX;
			width += APP_V2_TOC_STEP_PX
		) {
			...
			const fits = preparedTocEntries.every((entry) => {
				const measured = entry.prepared
					? layoutPreparedText(
							entry.prepared,
							textWidth,
							APP_V2_TOC_ITEM_LINE_HEIGHT_PX,
							{
								pagePath: location.pathname,
		...
		return APP_V2_TOC_MAX_WIDTH_PX;
	...
	return (
		<div style={{ textAlign: "left", marginTop: "14px" }}>
			...
			`}</style>
			<p style={BASE_TEXT_STYLE}>pourquoi ca marche ?</p>
			<div
				ref={layoutRef}
				className="appv2-why-layout"
				style={{
					display: "grid",
					gridTemplateColumns: `minmax(${APP_V2_TOC_MIN_WIDTH_PX}px, ${tocColumnWidthPx}px) minmax(0, 1fr)`,
					border: "1px solid #000000",
					...
					{article ? (
						...
						<>
							...
							<p style={{ ...BASE_TEXT_STYLE, margin: 0 }}>
								<Link to={APP_V2_DOCS_BASE_PATH} style={LINK_STYLE}>
									retour au debut de la documentation
								</Link>
							</p>
						</>
					)}
