...
begin
  ...
  select
    uc.card_id,
    case when o.origin_kind = 'foundation_seed' then 'foundation' else 'vocabulary' end as source,
    case
      when o.source_table = 'vocabulary_cards' then nullif(o.source_id, '')::uuid
      else null
    end as vocabulary_card_id,
    case
      when o.origin_kind = 'foundation_seed' then coalesce(nullif(o.source_id, ''), uc.card_id::text)::uuid
      else null
    end as foundation_card_id,
    case
      when o.origin_kind = 'foundation_seed' then 'foundation'
      else coalesce(nullif(uc.metadata->>'source_type', ''), 'collected')
    end as source_type,
    uc.state,
    uc.due_at,
    uc.reps,
  ...
  left join lateral (
    select co.origin_kind, co.source_table, co.source_id
...
begin
  ...
  due_rows as (
    select
      ...
      o.origin_kind,
      o.source_table as origin_source_table,
      ...
      (
        select jsonb_agg(
          jsonb_build_object(
            ...
            'vocabulary_card_id', case when dr.origin_source_table = 'vocabulary_cards' then nullif(dr.origin_source_id, '') else null end,
