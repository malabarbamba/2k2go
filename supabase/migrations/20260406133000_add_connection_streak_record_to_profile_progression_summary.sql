...
declare
	...
	v_connection_streak_record_days integer := 0;
...
begin
	...
	with connection_days as (
		select distinct uda.activity_date as activity_date
		from public.user_daily_activity uda
		where uda.user_id = p_target_user_id
	), grouped_days as (
		select
	...
	into v_connection_streak_record_days
	...
	select
		...
		v_connection_streak_record_days,
		v_review_current,
		v_review_target,
		v_review_progress,
		v_mastered_words,
