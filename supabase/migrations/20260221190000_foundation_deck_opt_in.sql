-- =====================================================
-- Foundation 2000 deck opt-in from dashboard
-- Date: 2026-02-21
-- Purpose:
--   1) New users start with no Foundation deck assignment
--   2) Foundation seeding runs only after explicit opt-in
--   3) Dashboard action can assign Foundation deck on demand
-- =====================================================

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS foundation_deck_enabled boolean NOT NULL DEFAULT false;
-- Preserve current behavior for already-active users.
UPDATE public.profiles p
SET foundation_deck_enabled = true
WHERE p.foundation_deck_enabled = false
  AND EXISTS (
    SELECT 1
    FROM public.user_card_state ucs
    WHERE ucs.user_id = p.user_id
...
BEGIN
  ...
  SELECT LEAST(200, GREATEST(0, COALESCE(p.new_cards_per_day, 20)))
  INTO v_daily_new_cap
  FROM public.profiles p
  WHERE p.user_id = v_user_id
  LIMIT 1;

  v_daily_new_cap := COALESCE(v_daily_new_cap, 20);
