...
const SIDEBAR_REMINDER_APP_PATH = "/app-legacy";
const DEFAULT_REVIEW_REMINDER_HANDOFF_PATH = "/calendar/review-reminders";
const DEFAULT_CALENDAR_NAME = "2k2go Review Reminders";
...
function canonicalizeReviewReminderAppUrl(normalizedUrl: string): string {
	try {
		...
		const shouldUseCanonicalAppPath =
			...
			normalizedPath === SIDEBAR_REMINDER_APP_PATH ||
			normalizedPath.startsWith(`${SIDEBAR_REMINDER_APP_PATH}/`);
...
export function resolveReviewReminderAppUrl(): string {
	...
	return `https://2k2go.github.io${DEFAULT_REMINDER_APP_PATH}`;
}
	...
	if (feed.enabled && feed.calendar_enabled && slots.length > 0) {
		for (const slot of slots) {
			...
			baseLines.push(
				"BEGIN:VEVENT",
				foldIcsLine(
					`UID:${escapeIcsText(`${feed.user_id}-${slot}@review-reminders.example`)}`,
				),
				foldIcsLine(
