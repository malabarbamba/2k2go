...
const DEFAULT_SITE_URL = "https://www.arabeimmersion.fr";
const DEFAULT_BRAND_NAME = "Arabe Immersion";
const DEFAULT_SUPPORT_EMAIL = "contact@arabeimmersion.fr";
const DEFAULT_FROM_EMAIL = "no-reply@arabeimmersion.fr";
const DEFAULT_EMAIL_LOGO_PATH = "/images/v2_full2.png";
const LEGACY_BRAND_PATTERN = /arabe\s*urgence/gi;
const LEGACY_FROM_DOMAIN_PATTERN = /@arabeurgence\.com$/i;
...
function normalizeBrandLabel(value: string): string {
	...
	return trimmed.replace(LEGACY_BRAND_PATTERN, DEFAULT_BRAND_NAME);
}
...
function normalizeFromEmailAddress(value: string): string {
	...
	if (LEGACY_FROM_DOMAIN_PATTERN.test(trimmed)) return DEFAULT_FROM_EMAIL;
	return trimmed;
}
...
export function getEmailBrandingFromEnv(
	...
	const fromName = coerceNonEmptyString(
		overrides.fromName ??
			...
			Deno.env.get("ARABEIMMERSION_SENDER_NAME") ??
			Deno.env.get("ARABEURGENCE_SENDER_NAME"),
		DEFAULT_BRAND_NAME,
	);
	...
	const fromEmail = normalizeFromEmailAddress(
		coerceNonEmptyString(
			overrides.fromEmail ??
				...
				Deno.env.get("ARABEIMMERSION_NON_REPLY_EMAIL") ??
				Deno.env.get("ARABEURGENCE_NON_REPLY_EMAIL"),
			DEFAULT_FROM_EMAIL,
		),
	);

	const supportEmail = coerceNonEmptyString(
		overrides.supportEmail ?? Deno.env.get("AUTH_EMAIL_SUPPORT"),
